...
const result = await agentbase.runAgent({
  message: "Visit example.com and extract the pricing table from the /pricing page",
  mode: "fast"
});
// Potential steps: 2-3
// Time: 4-6 seconds
...
| Data lookup | Flash/Fast | 2-3 | 2-5s | "Find user by ID" |
| File processing | Fast | 3-5 | 4-8s | "Parse CSV file" |
| Web scraping | Fast | 4-6 | 6-12s | "Extract pricing data" |
| Code generation | Fast/Max | 5-8 | 8-15s | "Create REST API" |
...
<CardGroup cols={2}>
  ...
  <Card title="Agent Modes" icon="gauge" href="/build/agent-modes">
    Learn more about Flash, Fast, and Max modes
  </Card>

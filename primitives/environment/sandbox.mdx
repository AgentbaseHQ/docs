...
// Fast mode - standard sandbox (default)
const fast = await agentbase.runAgent({
...
<Tip>
**Mode Selection**: Use `flash` mode for simple tasks, `fast` for standard workloads, and `max` only when you need advanced reasoning capabilities. This optimizes both cost and performance.
</Tip>
...
const modes = {
  ...
  standard: "fast",   // Most development and analysis tasks
...
- **Memory**: 2GB for flash/fast, 4GB for max mode

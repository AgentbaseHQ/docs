...
>
  ...
  `serverUrl`, the `serverUrl` is the `/sse` endpoint of the MCP server.
  ```json
  {
    "mcpServers": [
      { "serverName": "mcp-server-1", "serverUrl": "https://mcp-server-1.com/sse" },
      { "serverName": "mcp-server-2", "serverUrl": "https://mcp-server-2.com/sse" }
    ]
  }
  ```
</ParamField>

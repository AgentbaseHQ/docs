...
**Automatic & Free**: Message history and chat history are persisted by default with no cost. See [pricing details](/pricing/overview).
...
// Later - context is preserved
const session2 = await agentbase.runAgent({
  message: "Now create social media content for that campaign",
...
**Persistence**: Once created, computers persist with no current expiry date.
...
<CardGroup cols={3}>
  ...
  <Card title="Use Cases" icon="lightbulb" href="/getting-started/common-use-cases">
    Real-world examples leveraging persistence
  </Card>
</CardGroup>
